...
const ListExpense = () => {
  ...
  const filterList = expenseList
    .filter((l) => {
      return l.title.toLowerCase().includes(expenseQuery.title.toLowerCase());
    })
    .filter((l) => {
      return expenseQuery.type === "all" ? true : l.type === expenseQuery.type;
    })
    .filter((l) => {
      return expenseQuery.category === "all"
        ? true
  ...
  const sortedList = [...filterList].sort((a, b) => {
    ...
    if (expenseQuery.sort === "priceAsc") {
      return Number(a.amount) - Number(b.amount);
    }

    if (expenseQuery.sort === "priceDsc") {
      ...
      <Row className="mb-3 g-3 text-center">
        <Col xs={4}>
          <div className="p-2 rounded-3">
            <h6 className="text-success mb-1">Balance</h6>
            <h4 className="text-success mb-0">{balance}</h4>
          </div>
        </Col>
        <Col xs={4}>
          <div className="p-2 rounded-3">
            <h6 className="text-primary mb-1">Credit</h6>
            <h4 className="text-primary mb-0">{credit}</h4>
          </div>
        </Col>
        <Col xs={4}>
          <div className="p-2 rounded-3">
            <h6 className="text-danger mb-1">Debit</h6>
            <h4 className="text-danger mb-0">{debit}</h4>
          </div>
        </Col>
      </Row>
              ...
              sortedList.map((data, index) => {
                return (
                  <tr key={data.id}>
                    ...
                    <td>{data.category}</td>
                    <td>
                      <Badge bg={data.type === "credit" ? "success" : "danger"}>
                        {data.type}
                      </Badge>
                    </td>
                    <td>
